...
module Patir
  ...
  class CommandSequence
    ...
    def add_step(step, exit_strategy = :fail_on_error)
      ...
      unless %i[flunk_on_error fail_on_warning flunk_on_warning].include?(exit_strategy)
        exit_strategy = :fail_on_error
      end
  ...
  class CommandSequenceStatus
    ...
    def completed?
      ...
      @step_states.each do |state|
        if state[1][:status] == :error && state[1][:strategy] == :fail_on_error
          return true
        end
        if state[1][:status] == :warning && state[1][:strategy] == :fail_on_warning
          return true
        end
      end
      @step_states.each do |state|
        if state[1][:status] == :not_executed || state[1][:status] == :running
          return false
                                 end
      end
      true
